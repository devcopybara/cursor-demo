...
- **이메일 형식 검증** (`isValidEmail`): RFC 5322 기준 정규식으로 주소 형식을 검사합니다. 따옴표 로컬 파트, IP 도메인, `+tag` 형식을 지원하고, 잘못된 점(`.`) 배치는 거부합니다.
...
- `npm test`가 `node --test`로 이메일 관련 테스트 12건을 실행하도록 설정했습니다.
